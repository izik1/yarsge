...
impl Cpu {
    ...
    // Remarks: ----
    // Timing: Read, Read 
    fn ld_r16_d16(&mut self, reg: R16) -> i64 {
        let val = self.read_u16_cycle();
        match reg {
            R16::BC => self.regs.bc = val,
            R16::DE => self.regs.de = val,
            R16::HL => self.regs.hl = val,
            R16::SP => self.regs.sp = val,
        };
        8
    }
    ...
    // Timing: Write     
    fn ld_r16_a(&mut self, reg: R16) -> i64 {
