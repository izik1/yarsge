...
pub fn jp(cpu: &mut Cpu, jump: bool) -> i64 {
...
pub fn ld(cpu: &mut Cpu, dest: Reg, src: Reg) -> i64 {
    match (dest, src) {
        ...
        (Reg::HL, src) =>     {let val = cpu.regs.get_reg(&src); cpu.write_hl_cycle(val); 4}
        (dest, Reg::HL) =>    {let val = cpu.read_hl_cycle(); cpu.regs.set_reg(dest, val); 4}
        (dest, src) =>        {let val = cpu.regs.get_reg(&src); cpu.regs.set_reg(dest, val); 0}
    }
}
...
// Remarks: ----
// Timing: Read, Read
pub fn ld_r16_d16(cpu: &mut Cpu, reg: R16) -> i64 {
...
// Timing: Write
pub fn ld_r16_a(cpu: &mut Cpu, reg: R16) -> i64 {
    let a = cpu.regs.a;
    let reg = match reg {
        R16::BC => cpu.regs.bc,
        R16::DE => cpu.regs.de,
        R16::HL => {let v = cpu.regs.hl;cpu.regs.hl += 1;v}
        R16::SP => {let v = cpu.regs.hl;cpu.regs.hl -= 1;v}
    };
...
pub fn inc_16(cpu: &mut Cpu, reg: R16) -> i64 {
    cpu.update(4);
...
pub fn inc_8(cpu: &mut Cpu, reg: Reg) -> i64 {
    cpu.regs.f &= Flag::C.to_mask();
    let val;
    let cycles = match reg {
        Reg::HL => {val = cpu.read_hl_cycle(); cpu.write_hl_cycle(val.wrapping_add(1));8}
        r => {val = cpu.regs.get_reg(&r); cpu.regs.set_reg(r, val.wrapping_add(1));0}
    };

    if val == 0xFF {cpu.regs.set_flag(Flag::Z)};
    if get_hca(val, 1) {cpu.regs.set_flag(Flag::H)};
    cycles
}
...
// If a flags conditions aren't met, it is instead reset.
// Timing: "Instant" or "Read, Write"
pub fn dec_8(cpu: &mut Cpu, reg: Reg) -> i64 {
    let val;
    let cycles = match reg {
        Reg::HL => {val = cpu.read_hl_cycle(); cpu.write_hl_cycle(val.wrapping_sub(1));8}
        r => {val = cpu.regs.get_reg(&r); cpu.regs.set_reg(r, val.wrapping_sub(1));0}
    };

    cpu.regs.f = cpu.regs.f & Flag::C.to_mask();
    if val == 1 {cpu.regs.set_flag(Flag::Z)};
    cpu.regs.set_flag(Flag::N);
    if (val & 0xF) == 0 {cpu.regs.set_flag(Flag::H)};
    cycles
}
...
pub fn ld_r8_d8(cpu: &mut Cpu, reg: Reg) -> i64 {
    let val = cpu.read_ipc_cycle();
    4 + match reg {
        Reg::HL => {cpu.write_hl_cycle(val);4}
        r => {cpu.regs.set_reg(r, val);0}
    }
}
...
pub fn ld_a16_sp(cpu: &mut Cpu) -> i64 {
...
// Timing: Internal Delay
pub fn add_hl_reg16(cpu: &mut Cpu, reg: R16) -> i64 {
...
// Remarks: If r16 is HL, then HL increments after the operation. If r16 is SP it instead uses HL for the operation, and decrements HL after.
// Timing: Write
pub fn ld_a_r16(cpu: &mut Cpu, reg: R16) -> i64 {
    let reg = match reg {
        R16::BC => cpu.regs.bc,
        R16::DE => cpu.regs.de,
        R16::HL => {let v = cpu.regs.hl;cpu.regs.hl += 1;v}
...
// Remarks: ----
// Timing: Internal Delay.
pub fn dec_16(cpu: &mut Cpu, reg: R16) -> i64 {
    cpu.update(4);
...
// Remarks: ----
// Timing: Read, Read, Internal Delay
pub fn retc(cpu: &mut Cpu, jump: bool) -> i64 {
    cpu.update(4);
    if !jump {
        4
    } else {
...
// Remarks: ----
// Timing: Read, Read
pub fn pop(cpu: &mut Cpu, reg: R16) -> i64 {
...
pub fn call(cpu: &mut Cpu, jump: bool) -> i64 {
    let addr = cpu.read_u16_cycle();
    if jump {
        let pc = cpu.regs.pc;
...
pub fn push(cpu: &mut Cpu, reg: R16) -> i64 {
    let val = match reg {
...
// Remarks: ----
// Timing: Delay, Write, Write
pub fn rst(cpu: &mut Cpu, addr: u16) -> i64 {
    cpu.update(4);
    let pc = cpu.regs.pc;
    cpu.write_push_16_cycle(pc);
    cpu.regs.pc = addr;
...
// Remarks: ----
// Timing: Read, Read, Internal Delay
pub fn ret(cpu: &mut Cpu, reti: bool) -> i64 {
    let addr = cpu.read_pop_16_cycle();
    cpu.regs.pc = addr;
    if reti {
        cpu.ime = true;
...
// Remarks: ----
// Timing: Read, Write
pub fn ldh_a8_a(cpu: &mut Cpu) -> i64 {
...
// Remarks: ----
// Timing: Read, Write
pub fn ldh_c_a(cpu: &mut Cpu) -> i64 {
...
// Remarks: ----
// Timing: Read, Read
pub fn ldh_a_a8(cpu: &mut Cpu) -> i64 {
    let addr = 0xFF00 | cpu.read_ipc_cycle() as u16;
...
// Remarks: ----
// Timing: Read
pub fn ldh_a_c(cpu: &mut Cpu) -> i64 {
    let addr = 0xFF00 | cpu.regs.get_reg(&Reg::C) as u16;
    let val = cpu.read_cycle(addr);
    cpu.regs.set_reg(Reg::A, val);
    4
...
// Remarks: ----
// Timing: Read, Internal Delay
pub fn ld_hl_sp_r8(cpu: &mut Cpu) -> i64 {
...
pub fn ld_a_a16(cpu: &mut Cpu) -> i64 {
    let addr = cpu.read_u16_cycle();
    let val = cpu.read_cycle(addr);
    cpu.regs.set_reg(Reg::A, val);
    12
...
pub fn ld_a16_a(cpu: &mut Cpu) -> i64 {
    let addr = cpu.read_u16_cycle();
    let a = cpu.regs.get_reg(&Reg::A);
    cpu.write_cycle(addr, a);
    12
...
// Remarks: ----
// Timing: Internal delay
pub fn ld_sp_hl(cpu: &mut Cpu) -> i64 {
    let hl = cpu.regs.hl;
    cpu.update(4);
    cpu.regs.sp = hl;
    4
...
pub fn add_sp_r8(cpu: &mut Cpu) -> i64 {
    let r8 = cpu.read_ipc_cycle() as i8;
    cpu.update(8);
    cpu.regs.res_all_flags();
    if ((cpu.regs.sp & 0x0F) + (r8 as u16 & 0x0F)) > 0x0F {
...
pub fn rlc(cpu: &mut Cpu, reg: Reg) -> i64 {
    cpu.regs.res_all_flags();
    let val;
    let cycles = match reg {
        Reg::HL => {val = cpu.read_hl_cycle(); cpu.write_hl_cycle((val << 1) | (val >> 7));8}
        r => {val = cpu.regs.get_reg(&r); cpu.regs.set_reg(r, (val << 1) | (val >> 7));0}
    };
...
// Timing: "read, write" or instant.
pub fn rrc(cpu: &mut Cpu, reg: Reg) -> i64 {
    cpu.regs.res_all_flags();
    let val;
    let cycles = match reg {
        Reg::HL => {val = cpu.read_hl_cycle(); cpu.write_hl_cycle((val >> 1) | (val << 7));8}
...
// Remarks: Zero is set if the input was 0, Carry is set if bit 7 is set. If their conditions aren't satisfied, they are reset.
// Timing: "read, write" or instant.
pub fn rl(cpu: &mut Cpu, reg: Reg) -> i64 {
    let val;
    let carry_in = if cpu.regs.get_flag(Flag::C) {1} else {0};
    let res;
    let cycles = match reg {
        Reg::HL => {val = cpu.read_hl_cycle();res = (val << 1) | (carry_in); cpu.write_hl_cycle(res);8}
        r => {val = cpu.regs.get_reg(&r); res = (val << 1) | (carry_in); cpu.regs.set_reg(r, res);0}
    };
...
// Remarks: Zero is set if the input was 0, Carry is set if bit 0 is set. If their conditions aren't satisfied, they are reset.
// Timing: "read, write" or instant.
pub fn rr(cpu: &mut Cpu, reg: Reg) -> i64 {
    let val;
    let carry_in = if cpu.regs.get_flag(Flag::C) {0x80} else {0x00};
    let res;
    let cycles = match reg {
        Reg::HL => {val = cpu.read_hl_cycle();res = (val >> 1) | (carry_in); cpu.write_hl_cycle(res);8}
        r => {val = cpu.regs.get_reg(&r); res = (val >> 1) | (carry_in);cpu.regs.set_reg(r, res);0}
...
// Remarks: Zero is set if the input was 0, Carry is set if bit 7 is set. If their conditions aren't satisfied, they are reset.
// Timing: "read, write" or instant.
pub fn sla(cpu: &mut Cpu, reg: Reg) -> i64 {
    let val;
    let res;
    let cycles = match reg {
        Reg::HL => {val = cpu.read_hl_cycle(); res = val << 1; cpu.write_hl_cycle(res);8}
        r => {val = cpu.regs.get_reg(&r); res=val << 1; cpu.regs.set_reg(r, res);0}
    };
...
// Remarks: Zero is set if the input was 0, Carry is set if bit 0 is set. If their conditions aren't satisfied, they are reset.
// Timing: "read, write" or instant.
pub fn sra(cpu: &mut Cpu, reg: Reg) -> i64 {
    let val;
    let cycles = match reg {
        Reg::HL => {val = cpu.read_hl_cycle(); cpu.write_hl_cycle((val >> 1) | (val & 0x80));8}
        r => {val = cpu.regs.get_reg(&r); cpu.regs.set_reg(r, (val >> 1) | (val & 0x80));0}
    };
...
// Timing: "read, write" or instant.
pub fn swap(cpu: &mut Cpu, reg: Reg) -> i64 {
    let val;
    let cycles = match reg {
        Reg::HL => {val = cpu.read_hl_cycle(); cpu.write_hl_cycle((val << 4) | (val >> 4));8}
        r => {val = cpu.regs.get_reg(&r); cpu.regs.set_reg(r, (val << 4) | (val >> 4));0}
    };
...
// Remarks: Zero is set if the input was 0, Carry is set if bit 0 is set. If their conditions aren't satisfied, they are reset.
// Timing: "read, write" or instant.
pub fn srl(cpu: &mut Cpu, reg: Reg) -> i64 {
    let val;
    let cycles = match reg {
        Reg::HL => {val = cpu.read_hl_cycle(); cpu.write_hl_cycle((val >> 1));8}
        r => {val = cpu.regs.get_reg(&r); cpu.regs.set_reg(r, (val >> 1));0}
    };
...
// Timing: "read" or instant.
pub fn bit(cpu: &mut Cpu, reg: Reg, mask: u8) -> i64 {

    cpu.regs.f &= Flag::C.to_mask();
    cpu.regs.set_flag(Flag::H);
    match reg {
        Reg::HL => {if (cpu.read_hl_cycle() & mask) == 0 {cpu.regs.set_flag(Flag::Z)}; 4}
        r => {if (cpu.regs.get_reg(&r) & mask) == 0 {cpu.regs.set_flag(Flag::Z)}; 0}
    }
}
...
// Remarks: ----
// Timing: "read, write" or instant.
pub fn res(cpu: &mut Cpu, reg: Reg, mask: u8) -> i64 {
    match reg {
        Reg::HL => {let val = cpu.read_hl_cycle(); cpu.write_hl_cycle(val & mask); 8}
        r => {let val = cpu.regs.get_reg(&r); cpu.regs.set_reg(r, val & mask) ; 0}
    }
}
...
// Remarks: ----
// Timing: "read, write" or instant.
pub fn set(cpu: &mut Cpu, reg: Reg, mask: u8) -> i64 {
    match reg {
        Reg::HL => {let val = cpu.read_hl_cycle(); cpu.write_hl_cycle(val | mask); 8}
        r => {let val = cpu.regs.get_reg(&r); cpu.regs.set_reg(r, val | mask) ; 0}
    }
